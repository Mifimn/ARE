...
// --- Placeholder Data & Components for Bloodstrike ---

const upcomingCups = [
    { id: 601, title: 'Bloodstrike Strike Point Series - Week 1', format: 'Squads BR', entries: 28, date: '03 Nov', prize: '$500+', image: '/images/action_2.jpg' }, // Placeholder image
    { id: 602, title: 'Hot Zone Duos Challenge', format: 'Duos Hot Zone', entries: 44, date: '07 Nov', prize: '$200', image: '/images/action_1.jpg' }, // Placeholder image
];

const pastCups = [
    { id: 701, title: 'Bloodstrike Africa Launch Invitational', format: 'Squads BR', entries: 32, date: '20 Sep', prize: '$1000', image: '/images/lan_9.jpg', winner: 'Crimson Fury'}, // Placeholder image
];

const topTeams = [
    { rank: 1, name: 'Crimson Fury', points: 950, members: 4, logo: '/images/team_r.png' }, // Placeholder logos
    { rank: 2, name: 'Strike Force X', points: 910, members: 4, logo: '/images/team_x.png' },
    { rank: 3, name: 'Map Hunters', points: 850, members: 4, logo: '/images/team_y.png' },
    { rank: 4, name: 'Zone Controllers', points: 820, members: 4, logo: '/images/team_z.png' },
];

const allTeams = [
    { name: 'Rapid Responders', members: 4, logo: '/images/team_a.png' },
    { name: 'Ghost Operators', members: 4, logo: '/images/team_c.png' },
    { name: 'Zero Point Clan', members: 4, logo: '/images/team_o.png' },
];

// --- Placeholder Data for User's Joined Bloodstrike Tournaments ---
const myJoinedBloodstrikeTournaments = [
    { id: 601, title: 'Bloodstrike Strike Point Series - Week 1', date: '03 Nov', status: 'Upcoming', nextMatch: 'Check-in required 30 mins prior' },
    // Add more joined Bloodstrike tournaments here
];
// ------------------------------------------------------------------
...
const CupListItem = ({ cup, isPast = false }) => (
    ...
    >
        <img
            src={cup.image || '/images/bs_cup_thumb.jpg'} // Placeholder thumbnail
            alt="Cup Background"
...
// --- Content Components (Adapted for Bloodstrike) ---
...
const CupsContent = () => (
    <AnimatedSection delay={0} className="space-y-10">
        <h2 className="text-3xl font-bold text-primary-400 border-b-2 border-primary-500/30 pb-3 flex items-center">
            <Trophy size={28} className="mr-3" /> Bloodstrike Cups
        </h2>
        <p className="text-gray-300 text-lg">Upcoming official cups and tournament history for Bloodstrike.</p>
        {/* Upcoming */}
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {upcomingCups.length === 0 && <p className="text-gray-500 md:col-span-2">No upcoming Bloodstrike cups scheduled.</p>}
            </div>
        </div>
        {/* Past */}
...
const LeaderboardContent = () => (
    <AnimatedSection delay={0} className="space-y-8">
        <h2 className="text-3xl font-bold text-primary-400 border-b-2 border-primary-500/30 pb-3 flex items-center">
            <BarChart size={28} className="mr-3" /> Bloodstrike Leaderboard
        </h2>
        <p className="text-gray-300 text-lg">Top Bloodstrike teams competing on Africa Rise Esports.</p>
...
const TeamsContent = () => (
    <AnimatedSection delay={0} className="space-y-8">
        <h2 className="text-3xl font-bold text-primary-400 border-b-2 border-primary-500/30 pb-3 flex items-center"><Users size={28} className="mr-3" /> Bloodstrike Team Directory</h2>
        <p className="text-gray-300 text-lg">Find Bloodstrike teams or assemble your operators.</p>
        <AnimatedSection delay={100} className="bg-gradient-to-r from-green-600/30 to-dark-800/50 p-6 rounded-xl border border-green-500/50 shadow-lg flex items-center justify-between">
            <div><h3 className="text-2xl font-bold text-white mb-2">Assemble Your Strikers</h3><p className="text-green-200">Create your Bloodstrike team and dominate the zone.</p></div>
...
export default function BloodstrikePage() {
    ...
    // Placeholder Rundown data for Bloodstrike
    const recentMatchesRundown = [
        { winner: 'Reaper', loser: 'Viper', winnerScore: 1, loserScore: 0, winnerLogo: '/images/ava_m_7.png', loserLogo: '/images/ava_f_7.png' }, // Example player match
        { teams: ['/images/team_r.png', '/images/team_x.png', '/images/team_y.png', '/images/team_z.png'] }, // Example team logos from a Quads match
        { teams: ['/images/team_a.png', '/images/team_c.png', '/images/team_o.png'] }, // Example other match
    ];
            ...
            case 'rundown': default: return (
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="lg:w-2/3 space-y-10">
                        <AnimatedSection delay={100} className="relative group overflow-hidden bg-gradient-to-br from-red-700 via-rose-800 to-red-900 p-6 sm:p-8 rounded-2xl shadow-xl border-2 border-red-500/50">
                             <div className="absolute inset-0 bg-[url('/images/lan_9.jpg')] bg-cover opacity-[0.05] mix-blend-overlay"></div>
                             <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between"><h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-1 drop-shadow-lg">STRIKE POINT SERIES</h2><p className="text-red-200 text-base sm:text-lg mb-4 sm:mb-0">Official Bloodstrike Competitive Circuit</p><Link to="/event/strike-point" className="btn-primary bg-white text-red-800 hover:bg-red-100 font-bold py-3 px-6 rounded-xl flex items-center transition-colors text-base shadow-lg mt-4 sm:mt-0 transform hover:scale-105">EVENT INFO <ArrowRight size={20} className="ml-2" /></Link></div>
                        </AnimatedSection>
                    ...
                    <div className="lg:w-1/3 space-y-10">
                        ...
                        <AnimatedSection delay={300} className="card bg-dark-800 p-6 rounded-xl shadow-lg border border-dark-700">
                             ...
                             {myJoinedBloodstrikeTournaments.length > 0 ? (
                                <div className="space-y-4">
                                    {myJoinedBloodstrikeTournaments.map((tournament, index) => (
                                ...
                                <p className="text-gray-500 text-center py-4">You haven't joined any Bloodstrike tournaments yet.</p>
                             )}
                        ...
                        <AnimatedSection delay={400} className="card bg-dark-800 p-6 rounded-xl shadow-lg border border-dark-700">
                             <div className="flex justify-between items-center mb-4"><h3 className="text-2xl font-bold text-green-400">FIND TEAMMATES</h3><button className="bg-green-600/30 text-green-300 hover:bg-green-600/50 font-semibold py-1.5 px-4 rounded-lg text-sm flex items-center transition-colors"><PlusCircle size={16} className="mr-1" /> Post LFG</button></div>
                            <div className="bg-dark-900 p-4 rounded-lg flex justify-between items-center border border-dark-600 hover:border-green-500/50 transition-colors"><div><p className="text-base font-semibold text-white">BR Ranked Push</p><p className="text-sm text-gray-400 flex items-center"><Calendar size={14} className="mr-1" /> Squads - Need 1 More</p></div><div className="text-right"><p className="text-xl font-bold text-green-400 leading-none">3/4</p><p className="text-sm text-gray-500 leading-none">Players</p></div></div>
                        </AnimatedSection>
        ...
        <div className="bg-dark-900 text-white min-h-screen">
            <div className="max-w-full mx-auto space-y-10 pb-10">
                <AnimatedSection delay={0} className="relative h-64 sm:h-80 w-full overflow-hidden shadow-xl">
                    <img src="/images/action_2.jpg" alt="Bloodstrike Banner" className="absolute inset-0 w-full h-full object-cover object-center scale-105 blur-sm opacity-40"/>
                    ...
                    <div className="relative z-10 h-full flex flex-col justify-end max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 sm:pb-12">
                         <div className="flex items-center mb-4"><Gamepad2 className="w-10 h-10 sm:w-12 sm:h-12 mr-4 text-red-400 bg-dark-800/50 p-2 rounded-lg border border-red-500/30" /><h1 className="text-4xl md:text-6xl font-extrabold text-white drop-shadow-md">BLOODSTRIKE HUB</h1></div>
                         <p className="text-lg sm:text-xl text-gray-300 max-w-3xl">Engage in Bloodstrike competitions. Find tournaments, squads, and leaderboards.</p>
                    </div>
                </AnimatedSection>
