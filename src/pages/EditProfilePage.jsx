...
import { Save, X, User, Mail, Phone, MapPin, Check, Image as ImageIcon } from 'lucide-react'; // Added ImageIcon
import { Link } from 'react-router-dom';
import AnimatedSection from '../components/AnimatedSection';

// --- (Keep africanCountries, availableGames, avatar path generation as before) ---
const africanCountries = [ /* ... Full list ... */ ].sort();
...
// Placeholder Banner Images (Add your actual banner image paths)
const bannerImages = [
    '/images/lan_1.jpg',
    '/images/lan_2.jpg',
    '/images/lan_3.jpg',
...
export default function EditProfilePage() {
  const [formData, setFormData] = useState({
    ...
    banner: bannerImages[0], // Add banner state
  ...
  // --- (Keep existing handlers: handleChange, handleGameChange, handleGameDetailChange, handleAvatarSelect, handleSubmit as before) ---
   ...
   // New handler for banner selection
  ...
  return (
    ...
    <div className="bg-dark-900 text-white">
      ...
      <div className="max-w-4xl mx-auto py-6 sm:py-8 px-3 sm:px-6 lg:px-8">
        {/* Use AnimatedSection for the main card */}
        <AnimatedSection tag="div" className="card bg-dark-800 p-4 sm:p-6 md:p-8 rounded-xl shadow-2xl">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between mb-6 sm:mb-8 border-b border-dark-700 pb-4">
            <h1 className="text-2xl sm:text-3xl font-bold flex items-center text-primary-400 mb-4 sm:mb-0">
              <User className="mr-3" size={28} /> Edit Profile
            </h1>
            <div className="flex gap-3 w-full sm:w-auto">
              {/* Responsive buttons */}
              <Link to="/profile" className="btn-secondary flex items-center justify-center flex-1 sm:flex-initial text-sm py-2 px-3 sm:px-4">
                <X className="mr-1.5" size={16} /> Cancel
              </Link>
              <button onClick={handleSubmit} className="btn-primary flex items-center justify-center flex-1 sm:flex-initial text-sm py-2 px-3 sm:px-4">
                <Save className="mr-1.5" size={16} /> Save
              </button>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-10"> {/* Increased spacing */}
            ...
            {/* Banner Selection - Added Section */}
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 gap-3">
                    {bannerImages.map(bannerPath => (
                        <button
                            ...
                            className={`relative aspect-[16/6] rounded-md overflow-hidden border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-dark-800
                            ${formData.banner === bannerPath ? 'border-primary-500 scale-105 ring-2 ring-primary-500/50' : 'border-dark-600 hover:border-primary-400'}
                            `}
                        >
              ...
              <div className="flex flex-wrap gap-3 sm:gap-4 justify-center sm:justify-start">
                {allAvatars.map(avatarPath => (
                  <button
                    ...
                    className={`relative w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden border-2 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-dark-800
                      ${formData.avatar === avatarPath ? 'border-primary-500 scale-110 ring-2 ring-primary-500/50' : 'border-dark-600 hover:border-primary-400'}
                    `}
                  >
            ...
            <AnimatedSection delay={400}>
              ...
              {/* Responsive grid for game selection */}
                ...
                {availableGames.map(game => (
                  <label key={game} className={`flex items-center p-2.5 sm:p-3 rounded-md cursor-pointer transition-colors duration-150 ${formData.favoriteGames.includes(game) ? 'bg-primary-700/50 border border-primary-600' : 'bg-dark-700 hover:bg-dark-600 border border-transparent'}`}>
                    <input
                      ...
                      className="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-500 rounded mr-2 sm:mr-3 bg-dark-600 flex-shrink-0"
                    />
              ...
              {/* Conditionally render inputs for selected games */}
                  ...
                  {formData.favoriteGames.map(game => (
                    <AnimatedSection key={game} delay={100}>
                       <div className="p-4 bg-dark-800 rounded-lg border border-dark-600">
                          ...
                          {/* Responsive grid for IGN/UID */}
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                               <label htmlFor={`${game}-ign`} className="block text-xs font-medium text-gray-400 mb-1">In-Game Name (IGN)</label>
                               <input type="text" id={`${game}-ign`} value={formData.gameDetails[game]?.ign || ''} onChange={(e) => handleGameDetailChange(game, 'ign', e.target.value)} className="input-field input-field-sm" placeholder="e.g., YourGamerTag" />
                            </div>
                            <div>
                               <label htmlFor={`${game}-uid`} className="block text-xs font-medium text-gray-400 mb-1">User ID (UID)</label>
                               <input type="text" id={`${game}-uid`} value={formData.gameDetails[game]?.uid || ''} onChange={(e) => handleGameDetailChange(game, 'uid', e.target.value)} className="input-field input-field-sm" placeholder="e.g., 123456789" />
                            </div>
                          </div>
                       </div>
                    </AnimatedSection>
            ...
            <AnimatedSection delay={500}>
             ...
             {/* Form Actions (final save/cancel) */}
            <AnimatedSection delay={600} className="flex flex-col sm:flex-row justify-end gap-3 sm:gap-4 pt-6 border-t border-dark-700">
                <Link to="/profile" className="btn-secondary flex items-center justify-center py-2 px-4 text-sm order-2 sm:order-1"> {/* Order changes on small screens */}
                   <X className="mr-1.5" size={16} /> Cancel
                </Link>
                <button type="submit" className="btn-primary flex items-center justify-center py-2 px-4 text-sm order-1 sm:order-2"> {/* Order changes on small screens */}
...
// Helper style definition (optional, if you need smaller inputs)
// Add to your App.css or index.css if desired
/*
@layer components {
  .input-field-sm {
