...
const LeaderboardContent = () => (
    <AnimatedSection delay={0} className="space-y-6">
        ...
        <div className="bg-dark-800 p-6 rounded-xl shadow-inner">
            {/* Header Row */}
            <div className="flex justify-between items-center text-lg font-bold text-gray-400 mb-4 pb-3 border-b border-dark-700">
                <span className="w-1/12 text-center">#</span>
                <span className="w-5/12">Team Name</span>
                <span className="w-3/12 text-center hidden sm:block">Members</span>
            ...
            <div className="space-y-3">
                {topTeams.map((team) => (
                    <div key={team.rank} className="flex items-center p-4 bg-dark-700 rounded-lg hover:bg-dark-600 transition-colors border-l-4 border-yellow-500/50">

                        {/* Rank */}
                        <span className="w-1/12 text-center font-extrabold text-xl text-yellow-400">{team.rank}</span>

                        {/* Team Info */}
                        <div className="w-5/12 flex items-center space-x-4">
                            <img src={team.logo || '/images/team_placeholder.png'} alt={team.name} className="w-10 h-10 rounded-full object-cover border border-primary-500/50" />
                            <div>
                        ...
                        {/* Members */}
                        <div className="w-3/12 text-center text-gray-400 flex items-center justify-center hidden sm:flex">
                            <Users size={16} className="mr-1" />
                            <span>{team.members} / 5</span>
                        </div>

                        {/* Points */}
                        <span className="w-3/12 text-right font-bold text-xl text-yellow-400">{team.points.toLocaleString()}</span>
                    </div>
                ))}
    ...
    <AnimatedSection delay={0} className="space-y-6">
        ...
        <div className="space-y-4">
            {allTeams.map((team, index) => (
                <div key={index} className="flex items-center p-5 bg-dark-800 rounded-xl hover:bg-dark-700 transition-colors shadow-lg">

                    {/* Team Logo and Name */}
                    <div className="flex items-center w-5/12 space-x-4">
                        <img src={team.logo || '/images/team_placeholder.png'} alt={team.name} className="w-14 h-14 rounded-full object-cover border-2 border-primary-500/50" />
                        <p className="font-bold text-white text-xl">{team.name}</p>
                    </div>

                    {/* Members */}
                    <div className="w-4/12 flex items-center text-gray-400 text-base">
                    ...
                    <div className="w-3/12 text-right">
                        <Link 
                            ...
                            className="bg-primary-600 hover:bg-primary-700 text-white font-semibold py-2.5 px-5 rounded-lg text-base transition-colors flex items-center justify-end"
                        >
        ...
        return (
            <div className="flex flex-col lg:flex-row gap-12">
                ...
                <div className="lg:w-2/3 space-y-10">
                    <AnimatedSection delay={100} className="bg-purple-800/80 p-8 rounded-2xl shadow-xl flex items-center justify-between border-4 border-purple-600">
                        <div>
                            <h2 className="text-4xl font-extrabold text-white mb-1">MYTHIC 25</h2>
                            <p className="text-purple-200 text-lg">Featured Seasonal Championship Event</p>
                        </div>
                        <Link to="/event/mythic-25" className="bg-primary-600 hover:bg-primary-700 text-white font-bold py-3 px-8 rounded-xl flex items-center transition-colors text-lg shadow-lg">
                    ...
                    <AnimatedSection delay={400} className="bg-dark-800 p-6 rounded-xl shadow-lg border border-dark-700">
                        ...
                        <div className="border border-dark-700 p-4 rounded-lg mb-4">
                            <div className="flex items-center justify-between mb-1 text-base">
                                ...
                                <span className="font-bold text-red-500">{recentMatches[0].loserScore}</span>
                            </div>
                            <div className="flex items-center justify-between text-base pt-1 border-t border-dark-700 mt-2">
                                ...
                                <span className="font-bold text-green-400">{recentMatches[0].winnerScore}</span>
                            </div>
                        </div>
        ...
        <div className="pt-8 min-h-screen bg-dark-900 text-white">
            <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-10 space-y-10">
                ...
                <AnimatedSection delay={0} className="space-y-6">
                    ...
                    {/* Tabs Navigation */}
                    <nav className="flex space-x-8 text-xl font-semibold text-gray-400 border-b border-dark-700">
                        {navTabs.map((tab) => (
                            <button 
                                key={tab.name}
                                onClick={() => setActiveTab(tab.path)}
                                className={`py-3 transition-colors ${activeTab === tab.path ? 'text-primary-400 border-b-2 border-primary-400' : 'hover:text-white'}`}
                            >
                                {tab.name}
                            </button>
                        ))}
