...
const LeaderboardContent = ({ leaderboard, loading, error }) => (
    <AnimatedSection delay={0} className="space-y-8">
        ...
        <p className="text-gray-300 text-lg">
            Top Free Fire teams competing on Africa Rise Esports, ranked by win rate.
        </p>
        ...
        <div className="bg-dark-800 p-4 sm:p-6 rounded-xl shadow-inner border border-dark-700">
            ...
            <div className="space-y-3">
                ...
                {!loading && !error && leaderboard.length === 0 && (
                     <p className="text-center text-sm text-gray-500 py-10">No ranked teams found for Free Fire.</p>
                )}
            </div>
        </div>
    </AnimatedSection>
    ...
    <AnimatedSection delay={0} className="space-y-8">
        ...
        <AnimatedSection delay={200} className="space-y-6 pt-6">
            ...
            {!loading && !error && teams.length === 0 && (
                <p className="text-gray-500 text-center py-10">No Free Fire teams have been created yet.</p>
            )}
        </AnimatedSection>
    </AnimatedSection>
);
    ...
    useEffect(() => {
        ...
        const fetchUserData = async (userId) => {
            ...
            try {
                ...
                // 2. Get participant records for user's teams in Free Fire
                ...
                if (!participantRecords || participantRecords.length === 0) {
                    setLoadingMyData(false);
                    setMyTournaments([]);
                    return; // User has not participated in FF tourneys
                }
        ...
        <div className="bg-dark-900 text-white min-h-screen">
            <div className="max-w-full mx-auto space-y-10 pb-10">
                <AnimatedSection delay={0} className="relative h-64 sm:h-80 w-full overflow-hidden shadow-xl">
                    <img src="/images/free_lan.jpg" alt="Free Fire Banner" className="absolute inset-0 w-full h-full object-cover object-center scale-105 opacity-80"/>
