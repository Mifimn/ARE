...
const myJoinedTournaments = [
    { id: 1, title: 'African Mobile Legends League - S1', date: '01 Dec', status: 'Upcoming', nextMatch: 'vs Jungle Kings - 20:00' },
];
// --------------------------------------------------------
...
const CupListItem = ({ cup, isPast = false }) => {
    ...
    if (cup.format === 'round-robin-to-bracket') formatText = 'League & Playoffs';
    ...
    return (
        ...
        >
            <img
                src={cup.image || '/images/lan_7.jpg'} // Fallback image for ML
                alt="Cup Background"
            ...
            <>
                {cups.length > 0 ? (
                    ...
                    <p className="text-gray-500 md:col-span-2 text-center py-10">
                        No {isPast ? 'past' : 'upcoming'} Mobile Legends cups found.
                    </p>
                )}
            </>
        )}
...
const LeaderboardContent = ({ teams, loading, error }) => (
    <AnimatedSection delay={0} className="space-y-8">
        ...
        <p className="text-gray-300 text-lg">
            Top teams in Mobile Legends. Rankings based on official ARE tournament points.
             <br/>
        ...
        <div className="bg-dark-800 p-4 sm:p-6 rounded-xl shadow-inner border border-dark-700">
            ...
            <div className="space-y-3">
                ...
                {!loading && !error && teams.length === 0 && (
                     <p className="text-center text-sm text-gray-500 py-10">No teams found for Mobile Legends.</p>
                )}
            </div>
        </div>
    </AnimatedSection>
    ...
    useEffect(() => {
        const fetchAllData = async () => {
            ...
            const { data: cupData, error: cupError } = await supabase
                ...
                .eq('game', 'Mobile Legends') // --- UPDATED ---
                .eq('is_public', true)
                .order('start_date', { ascending: false });

            if (cupError) {
                console.error("Error fetching ML tournaments:", cupError);
            ...
            const { data: teamData, error: teamError } = await supabase
                ...
                .eq('tournaments.game', 'Mobile Legends'); // --- UPDATED ---

            if (teamError) {
                console.error("Error fetching ML teams:", teamError);
            ...
            default: return (
                <div className="flex flex-col lg:flex-row gap-8 lg:gap-12 px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="lg:w-2/3 space-y-10">
                        <AnimatedSection delay={100} className="relative group overflow-hidden bg-gradient-to-br from-blue-700 via-blue-800 to-indigo-900 p-6 sm:p-8 rounded-2xl shadow-xl border-2 border-blue-500/50">
                             <div className="absolute inset-0 bg-[url('/images/lan_7.jpg')] bg-repeat opacity-[0.03] mix-blend-overlay"></div>
                             <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center justify-between"><h2 className="text-3xl sm:text-4xl font-extrabold text-white mb-1 drop-shadow-lg">AML SEASON 1</h2><p className="text-blue-200 text-base sm:text-lg mb-4 sm:mb-0">Featured Seasonal Championship Event</p><Link to="/event/mythic-25" className="btn-primary bg-white text-blue-800 hover:bg-blue-100 font-bold py-3 px-6 rounded-xl flex items-center transition-colors text-base shadow-lg mt-4 sm:mt-0 transform hover:scale-105">GO TO EVENT <ArrowRight size={20} className="ml-2" /></Link></div>
                        </AnimatedSection>
                    ...
                    <div className="lg:w-1/3 space-y-10">
                        ...
                        <AnimatedSection delay={400} className="card bg-dark-800 p-6 rounded-xl shadow-lg border border-dark-700">
                            ...
                            <div className="bg-dark-900 p-4 rounded-lg flex justify-between items-center border border-dark-600 hover:border-green-500/50 transition-colors"><div><p className="text-base font-semibold text-white">Daily 5v5 Scrim</p><p className="text-sm text-gray-400 flex items-center"><Calendar size={14} className="mr-1" /> Best of 1 - Free Entry</p></div><div className="text-right"><p className="text-xl font-bold text-green-400 leading-none">19:00</p><p className="text-sm text-gray-500 leading-none">Today</p></div></div>
                        </AnimatedSection>
        ...
        <div className="bg-dark-900 text-white min-h-screen">
            <div className="max-w-full mx-auto space-y-10 pb-10">
                <AnimatedSection delay={0} className="relative h-64 sm:h-80 w-full overflow-hidden shadow-xl">
                    <img src="/images/lan_7.jpg" alt="Mobile Legends Banner" className="absolute inset-0 w-full h-full object-cover object-center scale-105 blur-sm opacity-40"/>
