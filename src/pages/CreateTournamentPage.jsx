...
import { PlusCircle, Calendar, Trophy, Users, MapPin, Save, X, AlertCircle, Info, Settings, Eye, FileText, Gamepad2 } from 'lucide-react'; // Added Gamepad2
...
// --- Game Specific Settings (Paste object and function from Step 1 Here) ---
...
const getInitialGameSettings = (gameName) => {
    ...
    // Add defaults for other potential settings like perspective (TPP/FPP) if needed
    return initial;
};
...
export default function CreateTournamentPage() {
  ...
  const defaultGame = availableGames[0]; // Use the first game from the updated list
  const [formData, setFormData] = useState({
    ...
    extraRules: '',
    // NEW: State for game-specific settings
  ...
  const handleChange = (e) => {
    ...
    setFormData(prevData => {
      ...
      // Handle game-specific settings nested state
      if (name.startsWith('gameSpecificSettings.')) {
        const key = name.split('.')[1]; // e.g., 'mode' or 'map'
        newData = {
          ...prevData,
      ...
      // If the game itself changed, update rules and reset game-specific settings
      if (name === 'game') {
        newData.rules = defaultGameRules[value] || '';
        newData.gameSpecificSettings = getInitialGameSettings(value); // Reset specific settings
      }
  ...
  const processSave = () => {
    ...
    alert('Tournament created successfully! Redirecting...');
  ...
  // --- (Styling helpers remain the same) ---
  ...
  const GameSpecificFields = ({ gameName, settings, onChange }) => {
    ...
    if (!options) return null; // No specific settings for this game
    ...
    return (
      <div className="p-4 bg-dark-800 rounded-lg border border-primary-500/30 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              {/* Add more conditional fields here (e.g., Perspective TPP/FPP) */}
          </div>
      </div>
    );
  };
