...
<td className="px-3 py-2 text-center font-extrabold text-primary-300">{r.tPoints}</td>
                            ...
                            <div className="text-center py-6 px-4">
                                <div className="bg-yellow-900/30 border border-yellow-700/50 text-yellow-300 p-4 rounded-lg space-y-2">
                                    ...
                                    <h4 className="font-semibold text-lg">Results Not Available</h4>
                                    <p className="text-sm">
                                        Results are either pending or failed to load. If the match is marked 'Completed',
                                        please check the Edge Function logs for `fetch-scrim-results`.
                                    </p>
                                </div>
                            </div>
                        )}
...
// --- ParticipantList component definition ---
...
<div className="card bg-dark-800 p-6 rounded-xl shadow-lg">
<h2 className="text-xl font-bold mb-4 flex items-center text-yellow-400"><Trophy className="mr-2"/> Prize Pool</h2>
...
const standings = {};
const allTeamsInStage = participants.filter(p => p.current_group); 
...
const [results, setResults] = useState([]); // <-- Will be populated by Edge Function
...
// --- State for selected tab/stage ---
const [activeTab, setActiveTab] = useState('rundown'); 


// --- UPDATED: Data Fetching (Using Edge Function for Results) ---
useEffect(() => {
...
try {
// 1. Fetch main tournament record (Direct Query)
...
setActiveTab(tournamentData.current_stage.toString()); 
} else {
setActiveTab('rundown');
}

// 2. Fetch all participants and matches with embedded participants (Direct Queries)
const [partRes, matchRes] = await Promise.all([
supabase.from('tournament_participants').select('*').eq('tournament_id', cupId),
supabase.from('tournament_matches').select('*, match_participants(participant_id)').eq('tournament_id', cupId).order('match_number', { ascending: true })
]);
...
setParticipants(partRes.data || []);
                ...
                // Process matches to flatten participant IDs
                let matchData = (matchRes.data || []).map(match => ({
                    ...match,
                    participant_ids: match.match_participants?.map(mp => mp.participant_id) || [] 
                }));
                setMatches(matchData);


// 3. 🚀 Fetch results via Edge Function (RLS Bypass) 🚀
                const { data: resultsFnData, error: resultsFnError } = await supabase.functions.invoke(
                    'fetch-scrim-results', // Function name is reused
                    { 
                        method: 'POST', 
                        body: { scrimId: cupId } // Send tournament ID
                    }
                );

                if (resultsFnError) {
                    console.error("Edge Function Invocation Error:", resultsFnError);
                    // Set an empty array and a friendly error message for the user
                    setResults([]);
                    // No need to crash the page, but log the error
                } else {
