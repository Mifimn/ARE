// src/pages/MobileLegendsScrimsPage.jsx
    ...
    return (
        ...
        >
            <img
                ...
                alt="Mobile Legends Scrim Background"
...
const ScrimsContent = ({ scrims, loading, error, isPast = false }) => (
    <AnimatedSection delay={0} className="space-y-6">
        {loading && ( <div className="flex justify-center items-center h-40"> <Loader2 className="w-8 h-8 text-primary-500 animate-spin" /> </div> )}
        {error && ( <div className="flex flex-col items-center justify-center h-40 bg-dark-800 p-4 rounded-lg"> <AlertCircle className="w-8 h-8 text-red-500 mb-2" /> <p className="text-red-400">Failed to load scrims.</p> </div> )}
        {!loading && !error && (
            <>
                {scrims.length > 0 ? (
                    ...
                    <p className="text-gray-500 md:col-span-2 text-center py-10"> No {isPast ? 'past' : 'upcoming'} scrims found for {GAME_NAME}. </p>
                )}
            </>
        )}
    </AnimatedSection>
...
export default function MobileLegendsScrimsPage() {
        ...
        <div className="bg-dark-900 text-white min-h-screen">
            <div className="max-w-7xl mx-auto space-y-10 pb-10">
                <AnimatedSection delay={0} className="relative h-64 sm:h-80 w-full overflow-hidden shadow-xl">
                    <img src={GAME_BANNER_URL} alt="Mobile Legends Scrims Banner" className="absolute inset-0 w-full h-full object-cover object-center scale-105 opacity-80"/>
